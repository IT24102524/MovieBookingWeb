...
<%
    // Get the movie object passed from servlet
    main.java.com.movieticket.model.Movie movie = (main.java.com.movieticket.model.Movie) request.getAttribute("movie");
%>
...
<title>Book Ticket - <c:out value="${movie.title}" /></title>
...
<div class="booking-container">
  <h2>Book Your Ticket for <c:out value="${movie.title}" /></h2>
  ...
  <form action="${pageContext.request.contextPath}/bookings" method="post" novalidate>
    ...
    <select id="showtime" name="showtime" required disabled>
...
<script>
  const showtimes = [
    <% if (movie != null) {
      for (main.java.com.movieticket.model.Showtime st : movie.getShowtimes()) {
        out.print("{date: '" + st.getDate() + "', time: '" + st.getTime() + "'},");
      }
    } %>
  ];
  ...
  function updateShowtimes() {
    ...
    timeSelect.innerHTML = '<option value="" selected disabled>Select a showtime</option>';
    ...
    const filteredTimes = showtimes.filter(st => st.date === selectedDate).map(st => st.time);
    ...
    timeSelect.disabled = filteredTimes.length === 0;
  }
